...
async def parse_file_id(message: "Mesaj") -> Optional[FileId]:
    media = get_media_from_message(message)
    if media:
        return FileId.decode(media.file_id)

async def parse_file_unique_id(message: "Mesaj") -> Optional[str]:
    media = get_media_from_message(message)
    if media:
...
def get_media_from_message(message: "Mesaj") -> Any:
    media_types = (
        "audio",
        "document",
        "photo",
...
def get_media_mime_type(m):
    media = get_media_from_message(m)
    return getattr(media, "mime_type", "Yok/bilinmiyor")
...
async def gen_link(m: Message,log_msg: Messages, from_channel: bool):
    ...
    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🖥KISA LİNK", url=page_link), InlineKeyboardButton("İNDİR 📥", url=stream_link)]])

    if from_channel:
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🖥KISA LİNK", url=page_link), InlineKeyboardButton("İNDİR 📥", url=stream_link)]])
    else:
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🖥KISA LİNK", url=page_link), InlineKeyboardButton("İNDİR 📥", url=stream_link)],
            [InlineKeyboardButton("❌ Link'i Sil", callback_data=f"msgdelconf2_{log_msg.message_id}_{get_media_file_unique_id(log_msg)}")]])
